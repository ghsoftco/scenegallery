...
void Controller::InitAssets()
{
    ...
    _assets.sphere.CreateSphere(1.0f, 4);
    _assets.sphere.SimplifyToPercentFaces(0.7f);
    _assets.sphere.UnIndex();
    _assets.sphere.GCNormals();

    _assets.font.Init(*_state->graphics, "Verdana", 20, FW_BOLD);
    _assets.psBase.Init(*_state->graphics, "TextureColor.ps");
    _assets.vsBase.Init(*_state->graphics, "TextureColor.vs");
...
void Controller::Frame()
{
    ...
    _assets.sphere.ApplyMatrix(Matrix4::RotationZ(_state->timer.SPF() * Math::PIf));
